...
<section>
	<h1 class="mb-2 text-2xl font-bold">Manage Matches</h1>
	<p class="mb-6 max-w-2xl text-muted-foreground">
		Create or auto generate matches in bulk using the round robin format for your tournaments.
	</p>
	{#if tournaments.length === 0}
		<div class="flex flex-col items-center justify-center rounded-lg border border-dashed p-8">
			...
			<p class="text-center text-sm text-muted-foreground">
				Please make sure the tournaments are created and finalized to manage matches.
			</p>
		</div>
	{:else}
