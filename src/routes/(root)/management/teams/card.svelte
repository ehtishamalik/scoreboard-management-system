<script lang="ts">
  ...
  const nameMap = $derived(new Map(participations.map((p) => [p.id, p.user.name])));
  ...
  const handleSubmitTeams = async () => {
    ...
    teams.forEach((team) => {
      if (tournament.isSinglePlayer) {
        if (team.name && team.playerOneId) {
          teamToSubmit.push({
            name: team.name.trim(),
            participationPlayerOne: team.playerOneId,
            participationPlayerTwo: team.playerOneId,
  ...
  <Card.Header>
    <Card.Title class="mb-2 flex items-center justify-between text-xl">
      ...
      <div class="flex items-center gap-2">
        <Button variant="outline" size="icon" disabled={isLoading} onclick={handleLoad}>
          <RefreshIcon class={{ 'animate-spin': isLoading }} />
        </Button>
        <Button variant="outline" size="icon" disabled={isLoading} onclick={handleAddTeam}>
          <PlusIcon />
        </Button>
            ...
            <AlertDialog.Content>
              <AlertDialog.Header>
                ...
                <AlertDialog.Description>
                  ...
                  associated matches. New teams will be saved, and if matches were already
                  generated, you'll need to regenerate them.
                </AlertDialog.Description>
              </AlertDialog.Header>
              <AlertDialog.Footer>
