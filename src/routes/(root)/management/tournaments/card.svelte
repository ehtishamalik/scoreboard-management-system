<script lang="ts">
  ...
  const handleStartedAlert = () => {
    if (!tournament.isFinalized) {
      toast.error('Cannot start tournament', {
        description: 'Tournament must be finalized before starting.'
      });
      return;
    }

    isStartedAlertOpen = true;
  };
  ...
  const handleCompleteAlert = () => {
    if (!tournament.isFinalized || !tournament.isStarted) {
      toast.error('Cannot complete tournament', {
        description: 'Tournament must be finalized and started before completing.'
      });
      return;
    }
...
<Card.Root>
  <Card.Header>
    ...
    <Card.Description class="sr-only">Manage tournament dates and status</Card.Description>
  </Card.Header>
  <Card.Content class="flex flex-col items-start gap-4 sm:max-w-md sm:flex-row sm:items-center">
  ...
  <Card.Footer class="justify-end gap-4">
    {#if !tournament.isFinalized}
      ...
      >
        ...
        <AlertDialog.Content>
          <AlertDialog.Header>
            <AlertDialog.Title>Finalize Tournament</AlertDialog.Title>
    ...
    {#if !tournament.isStarted}
      <AlertDialog.Root
        open={isStartedAlertOpen}
        onOpenChange={(open) => (isStartedAlertOpen = open)}
      >
        <AlertDialog.Trigger>
          {#snippet child({ props })}
            <Button
        ...
        <AlertDialog.Content>
          <AlertDialog.Header>
            <AlertDialog.Title>Start Tournament</AlertDialog.Title>
            <AlertDialog.Description>
              This action cannot be undone. After starting, you will no longer be able to manage
              participations, adjust matches with drag and drop or create teams.
            </AlertDialog.Description>
          </AlertDialog.Header>
          <AlertDialog.Footer>
            <AlertDialog.Cancel>Cancel</AlertDialog.Cancel>
            <AlertDialog.Action onclick={handleStarted}>Continue</AlertDialog.Action>
          </AlertDialog.Footer>
        </AlertDialog.Content>
      </AlertDialog.Root>
    {/if}
    {#if tournament.isActive}
      ...
      >
        ...
        <AlertDialog.Content>
          <AlertDialog.Header>
            <AlertDialog.Title>Complete Tournament</AlertDialog.Title>
            <AlertDialog.Description>
