...
export const zodTournamentSchema = z.object({
  ...
  isSinglePlayer: z.boolean().default(false).optional(),
  isStarted: z.boolean().default(false).optional()
});
...
export const zodMatchSeriesSchema = z.object({
  tournamentId: z.string().min(1, 'Tournament ID is required'),
  team1Id: z.string().min(1, 'Team 1 ID is required'),
  team2Id: z.string().min(1, 'Team 2 ID is required'),
  type: z.enum(['SEMIFINAL', 'FINAL']).default('SEMIFINAL').optional(),
  winnerId: z.string().optional().nullable(),

  game1Team1Points: z.number().int().min(0).default(0),
  game1Team2Points: z.number().int().min(0).default(0),
  game2Team1Points: z.number().int().min(0).default(0),
  game2Team2Points: z.number().int().min(0).default(0),
  game3Team1Points: z.number().int().min(0).default(0),
  game3Team2Points: z.number().int().min(0).default(0),

  playedDate: z.iso.date()
});
