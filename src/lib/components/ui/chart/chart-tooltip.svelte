<script lang="ts">
  ...
  import { getChartContext, Tooltip as TooltipPrimitive, type Placement } from 'layerchart';
  ...
  const formattedLabel = $derived.by(() => {
    ...
    if (!labelKey && typeof label === 'string') {
      ...
      const tooltipValue =
        tooltipData && typeof tooltipData === 'object' && labelKey in tooltipData
          ? (tooltipData as any)[labelKey]
          : undefined;
      value = tooltipValue ?? itemConfig?.label ?? dataLabel;
    } else {
      value = dataLabel;
    }
...
<TooltipPrimitive.Root variant="none" {x} {y} {anchor} {xOffset} {yOffset}>
  {#snippet children()}
    <div
      bind:this={ref}
      class={cn(
