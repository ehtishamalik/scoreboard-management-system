...
export const tournament = pgTable('tournament', {
  ...
  isSinglePlayer: boolean('is_single_player').default(false).notNull(),
...
export const matchSeries = pgTable('match_series', {
  ...
  team2Id: text('team2_id')
    .notNull()
    .references(() => teams.id, { onDelete: 'cascade' }),
  type: MATCH_TYPE('type').default('SEMIFINAL').notNull(),
  winnerId: text('winner_id').references(() => teams.id),

  // --- Game breakdown ---
  game1Team1Points: integer('game1_team1_points').default(0).notNull(),
  game1Team2Points: integer('game1_team2_points').default(0).notNull(),

  game2Team1Points: integer('game2_team1_points').default(0).notNull(),
  game2Team2Points: integer('game2_team2_points').default(0).notNull(),

  game3Team1Points: integer('game3_team1_points').default(0).notNull(),
  game3Team2Points: integer('game3_team2_points').default(0).notNull(),
...
export const matchSeriesRelations = relations(matchSeries, ({ one, many }) => ({
  tournament: one(tournament, {
    fields: [matchSeries.tournamentId],
    references: [tournament.id]
  }),

  team1: one(teams, {
    fields: [matchSeries.team1Id],
    references: [teams.id]
  }),

  team2: one(teams, {
    fields: [matchSeries.team2Id],
    references: [teams.id]
  }),

  winner: one(teams, {
    fields: [matchSeries.winnerId],
    references: [teams.id]
  }),
